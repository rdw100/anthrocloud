    ...
    public class AnthroFormBase : ComponentBase
    {
        ...
        protected async Task HandleValidSubmitAsync()
        {
            ...
            if (isValid)
            {
                ...
                Tuple<double, double> bfaTuple = await AnthroStatsService.GetBFA(formModel.MUAC, formModel.Age.TotalDays.ToString(), formModel.Sex);
                formModel.BfaZscore = bfaTuple.Item1;
                formModel.BfaPercentile = bfaTuple.Item2;

                Tuple<double, double> hcaTuple = await AnthroStatsService.GetHCA(formModel.MUAC, formModel.Age.TotalDays.ToString(), formModel.Sex);
