        ...
        protected async Task HandleValidSubmitAsync()
        {
            ...
            try
            {
                ...
                if (isValid)
                {
                    ...
                    Tuple<double, double> wfaTuple = await AnthroService.GetWFA(FormModel.FormInputs.Weight, FormattableString.Invariant($"{FormModel.FormInputs.Age.TotalDays}"), FormModel.FormInputs.Sex).ConfigureAwait(false);
                    FormModel.FormOutputs.WfaZscore = wfaTuple.Item1;
                    FormModel.FormOutputs.WfaPercentile = wfaTuple.Item2;

                    Tuple<double, double> muacTuple = await AnthroService.GetMUAC(FormModel.FormInputs.MUAC, FormattableString.Invariant($"{FormModel.FormInputs.Age.TotalDays}"), FormModel.FormInputs.Sex).ConfigureAwait(false);
                    FormModel.FormOutputs.MuacZscore = muacTuple.Item1;
                    FormModel.FormOutputs.MuacPercentile = muacTuple.Item2;

                    Tuple<double, double> bfaTuple = await AnthroService.GetBFA(FormModel.FormInputs.BMI, FormattableString.Invariant($"{FormModel.FormInputs.Age.TotalDays}"), FormModel.FormInputs.Sex).ConfigureAwait(false);
                    FormModel.FormOutputs.BfaZscore = bfaTuple.Item1;
                    FormModel.FormOutputs.BfaPercentile = bfaTuple.Item2;

                    Tuple<double, double> hcaTuple = await AnthroService.GetHCA(FormModel.FormInputs.HeadCircumference, FormattableString.Invariant($"{FormModel.FormInputs.Age.TotalDays}"), FormModel.FormInputs.Sex).ConfigureAwait(false);
                    FormModel.FormOutputs.HcaZscore = hcaTuple.Item1;
                    FormModel.FormOutputs.HcaPercentile = hcaTuple.Item2;

                    Tuple<double, double> hfaTuple = await AnthroService.GetHFA(FormModel.FormInputs.LengthHeight, FormattableString.Invariant($"{FormModel.FormInputs.Age.TotalDays}"), FormModel.FormInputs.Sex).ConfigureAwait(false);
                    FormModel.FormOutputs.HfaZscore = hfaTuple.Item1;
                    FormModel.FormOutputs.HfaPercentile = hfaTuple.Item2;

                    Tuple<double, double> lfaTuple = await AnthroService.GetLFA(FormModel.FormInputs.LengthHeight, FormattableString.Invariant($"{FormModel.FormInputs.Age.TotalDays}"), FormModel.FormInputs.Sex).ConfigureAwait(false);
                    FormModel.FormOutputs.LfaZscore = lfaTuple.Item1;
                    FormModel.FormOutputs.LfaPercentile = lfaTuple.Item2;

                    Tuple<double, double> sfaTuple = await AnthroService.GetSFA(FormModel.FormInputs.SubscapularSkinFold, FormattableString.Invariant($"{FormModel.FormInputs.Age.TotalDays}"), FormModel.FormInputs.Sex).ConfigureAwait(false);
                    FormModel.FormOutputs.SsfZscore = sfaTuple.Item1;
                    FormModel.FormOutputs.SsfPercentile = sfaTuple.Item2;

                    Tuple<double, double> tfaTuple = await AnthroService.GetTFA(FormModel.FormInputs.TricepsSkinFold, FormattableString.Invariant($"{FormModel.FormInputs.Age.TotalDays}"), FormModel.FormInputs.Sex).ConfigureAwait(false);
                    FormModel.FormOutputs.TsfZscore = tfaTuple.Item1;
                    FormModel.FormOutputs.TsfPercentile = tfaTuple.Item2;

                    Tuple<double, double> wfhTuple = await AnthroService.GetWFH(FormModel.FormInputs.Weight, FormModel.FormInputs.LengthHeight, FormModel.FormInputs.Sex).ConfigureAwait(false);
                    FormModel.FormOutputs.WfhZscore = wfhTuple.Item1;
                    FormModel.FormOutputs.WfhPercentile = wfhTuple.Item2;

                    Tuple<double, double> wflTuple = await AnthroService.GetWFL(FormModel.FormInputs.Weight, FormModel.FormInputs.LengthHeight, FormModel.FormInputs.Sex).ConfigureAwait(false);
